...
1) click_function.R: handles interaction with the webpage (scrolling and clicking buttons).
2) download_files.R: downloads the dataset files for each city and organizes them.
3) decompression.R: decompresses the downloaded archives and standardizes the resulting files (renaming and replacing where needed).
