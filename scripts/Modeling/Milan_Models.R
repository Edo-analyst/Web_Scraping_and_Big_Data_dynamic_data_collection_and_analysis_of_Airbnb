...
lambda.grid = exp(seq(-8, -4, length = 150))
...
lambda.grid_ad = exp(seq(-6, 0, length = 150))
mod.lasso_ad=cv.glmnet(x[,-1], sss$lp,alpha=1, 
...
lambda.grid_el = exp(seq(-6, 1, length = 150))
elast_net=cv.glmnet(x[,-1], sss$lp,alpha=0.5, 
...
# This can be done using a validation set or cross-validation.
...
#' tree.control controls how far to grow the tree
...
#' model overfits, so we want to prune it
#' use CV to prune
...
#' adjust it using CV
str(prune)
#' list indexed by size with error measures (we focus on deviance)
...
#' Estimates step functions (not smooth), steep relationships are hard to capture
...
#' Only returns mean CV error, difficult to use 1se CV
